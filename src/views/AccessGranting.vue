<template>
      ...
      <div class="content">
        ...
        <div class="access-granting-content">
          ...
          <div class="email-input-container">
            <textarea v-model="emails" placeholder="Введите почты через запятую или на каждой строке"></textarea>
          </div>
          <button class="send-button" @click="sendEmails">{{ t('send') }}</button>
  
          <h2>{{ t('adminEmail') }}</h2>
          <div class="admin-email-container">
            <input type="email" v-model="adminEmail" placeholder="Введите почту администратора" />
          </div>
          <button class="send-button" @click="sendAdminEmail">{{ t('send') }}</button>
        </div>
      </div>
    </div>
  </template>
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      return { t, goToHome, goToSettings, emails, adminEmail, sendEmails, sendAdminEmail };
    }
  });
  </script>
